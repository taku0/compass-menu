...
function activateWindowRelative(offset) {
    ...
    var windowArray = Array.prototype.slice.call(windows);
...
function showFrameInfo() {
    ensureFrameScriptLoaded();

    var window = windowUtils.getMostRecentBrowserWindow();

    // requests the frame script for document object and outerWindowID.
    // This ends up with doShowFrameInfo function below.
    window.messageManager
        .broadcastAsyncMessage("compass_menu@tatapa.org:showFrameInfo");
}

/**
 * Shows the current frame information.
 */
function doShowFrameInfo(message) {
    var outerWindowID = message.data.outerWindowID;
    var doc = message.objects.document;
    var window = windowUtils.getMostRecentBrowserWindow();

    window.BrowserPageInfo(doc, null, null, outerWindowID);
}
...
var frameScriptLoaded = false;

function ensureFrameScriptLoaded() {
    if (frameScriptLoaded) {
        return;
    }

    // loads new frame scripts again.
    globalMessageManager.loadFrameScript(data.url('frameScript.js'), true);

    frameScriptLoaded = true;
}

// removes previously loaded frame scripts. This should be done at the shutdown
// function at bootstrap.js but it is not provided for addons.
globalMessageManager.removeDelayedFrameScript(data.url('frameScript.js'));

// Since we cannot unload previously loaded frame scripts, disables them.
globalMessageManager.broadcastAsyncMessage("compass_menu@tatapa.org:disableFrameScript");

let frameMessageListeners = {
    showFrameInfo: doShowFrameInfo
};

for (let key in frameMessageListeners) {
    globalMessageManager.addMessageListener("compass_menu@tatapa.org:" + key,
                                            frameMessageListeners[key]);
}
